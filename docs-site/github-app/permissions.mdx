...
| --- | --- | --- |
| **Checks** | `read & write` | Create check runs on the pull request with a findings summary, merge readiness score, and pass/fail status. |
| **Contents** | `read & write` | **Read:** the pull request diff, file contents used to ground and verify findings against the real code, and `.mergewatch.yml` from the repository root. **Write:** resolving and unresolving review threads MergeWatch itself created — that is the only thing the write half is used for. GitHub offers no narrower permission that covers thread resolution. Declining this permission is supported: reviews, findings, inline comments and check runs all continue to work, and only thread tidying is lost. |
...
<CardGroup cols={3}>
  <Card title="No commits, branches, or files" icon="lock">
    MergeWatch never pushes commits, creates or deletes branches or tags, modifies files, opens or merges pull requests, or changes repository settings. `Contents: read & write` is granted only because GitHub routes review-thread resolution through it.
  </Card>
...
| `check_suite` | Listens for `rerequested`. GitHub's **Re-run all checks** button fires `check_suite.rerequested`, *not* `check_run.rerequested` — without this event that button silently does nothing. |
...
<Accordion title="Can MergeWatch modify my code?">
  No. MergeWatch requests `contents: read & write`, but the write half is used for exactly one thing — resolving and unresolving review threads MergeWatch itself created. It cannot push commits, create branches, or modify any file. The other write operations are posting review comments (`pull_requests: read & write`) and creating check runs (`checks: read & write`). Declining the contents write permission is supported; only thread tidying is lost.
</Accordion>
