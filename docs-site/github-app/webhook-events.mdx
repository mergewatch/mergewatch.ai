...
| `check_suite` | `rerequested` | GitHub's **Re-run all checks** button fires `check_suite.rerequested`, not `check_run.rerequested`. MergeWatch confirms the suite actually contains its own check run before acting, and ignores suites that do not. |
