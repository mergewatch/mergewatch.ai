...
<Info>
There is no standalone `mergewatch` CLI (`npx mergewatch@latest`). MergeWatch is managed with Docker Compose for self-hosted deployments and with `pnpm` + `sam` scripts for deploying the SaaS stack from the [mergewatch.ai repo](https://github.com/mergewatch/mergewatch.ai).
</Info>
...
Deploying the AWS stack (API Gateway + Lambda + DynamoDB + Bedrock) is done by cloning [mergewatch.ai](https://github.com/mergewatch/mergewatch.ai) and running the scripts defined in `package.json`.

```bash
git clone https://github.com/mergewatch/mergewatch.ai.git
cd mergewatch.ai
pnpm install
```
