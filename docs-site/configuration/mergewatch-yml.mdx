...
| `maxFindings` | `number` | `25` | Maximum number of model-generated findings posted per review, applied after filtering and keeping the highest-ranked. Prevents noisy reviews on large PRs. Findings from [custom agents](/dashboard/custom-agents) are exempt and post in addition to this cap. |
