...
- **PR exceeds `rules.maxFiles` limit** — PRs with more changed files than the configured `maxFiles` value (default: 50) are skipped with a visible check run explaining why. Only an `autoReview: false` skip is silent — every other skip kind still surfaces a check run.
