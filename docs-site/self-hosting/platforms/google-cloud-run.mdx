...
description: "Deploy MergeWatch to Google Cloud Run backed by Cloud SQL for Postgres, covering prerequisites, service configuration, and webhook URL setup."
---
...
Set `LLM_PROVIDER` to one of `anthropic`, `bedrock`, `litellm`, or `ollama`. There is no `openai` provider — reach OpenAI-compatible endpoints through `litellm`. For the default Anthropic provider, you also need `ANTHROPIC_API_KEY`.
