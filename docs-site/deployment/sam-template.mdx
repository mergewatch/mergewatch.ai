...
<Note>
The project uses a `scripts/deploy.sh` wrapper around `sam build && sam deploy` — invoke it with `pnpm run deploy:dev` / `deploy:staging` / `deploy` from the [mergewatch.ai repo](https://github.com/mergewatch/mergewatch.ai). You only need to invoke SAM directly if you are customizing deployment flags.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="DynamoDB">
    ...
    Specific actions rather than `dynamodb:*`, and scoped by `Resource` to the two MergeWatch
    table ARNs — the grant cannot reach any other table in the account. `Scan` is used where a
    lookup has no partition key to query by; `DeleteItem` backs record removal such as revoking
    an API key.
  </Accordion>
