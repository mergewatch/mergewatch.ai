...
// Dedupe the "invalid env pricing" warn by the specific (model, prices) tuple,
// so fixing one misconfiguration and introducing a different one still warns.
// Naturally bounded — these are process-static env vars, so it holds ~1 entry.
const warnedInvalidPricingKeys = new Set<string>();
...
function envModelPricing(): PricingMap | undefined {
  ...
  if (!pricing && model && (hasValue(input) || hasValue(output))) {
    const key = `${model}\0${input ?? ''}\0${output ?? ''}`;
    if (!warnedInvalidPricingKeys.has(key)) {
      warnedInvalidPricingKeys.add(key);
      console.warn(
...
export async function processReviewJob(
  ...
  try {
    ...
    // repo scope, matching path/language targeting), then run them in union
    // with the repo's `.mergewatch.yml` customAgents (org wins on name clash).
    ...
    // #235 — authors may triage a blocking org agent's finding, but it's not
    // silently dropped: every triage is already recorded in the disposition
    // store (admin-visible on the Accuracy dashboard); here we emit an explicit
    // blocking-tagged signal so admins can see a policy finding was suppressed.
    ...
    const result = await runReviewPipeline(
      {
        ...
        fileFetchOptions,
        groundingFetch,
        customAgents: unionCustomAgents(selectedOrgAgents, config.customAgents),
        tone: config.ux.tone,
    ...
    // #235 — blocking org agents: a critical finding from a *blocking* org
    // agent gates the merge regardless of the overall score — REQUEST_CHANGES
    // + a failing check run (see Step C below).
